            ...
            try:
                ...
                if message['request'] == 'login' and self not in connectedClients:
                    ...
                    if not newUsername:
                        ...
                        self.request.sendall(self.makePayload('SERVER', 'error', 'Invalid username!'))
                    ...
                    self.request.sendall(self.makePayload('SERVER', 'info', connectedClients.values()))

                elif message['request'] == 'help' and self in connectedClients:
                    self.request.sendall(self.makePayload('SERVER', 'info', help_text))

                else:
                    self.request.sendall(self.makePayload('SERVER', 'error', 'Not a valid argument'))
